def palindromes(sentence):
	...
	for i in range(len(sentence)):
		...
		for charIndexer in range(i+2, (len(sentence)+1)):

			if (sentence[charIndexer-1] == currentChar):

				currentSentence = sentence[i:charIndexer]
				reverseSentence = currentSentence[::-1]

				if (currentSentence == reverseSentence):
					nPalindromes += 1
